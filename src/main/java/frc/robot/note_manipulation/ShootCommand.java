...
public class ShootCommand extends SequentialCommandGroup {
    public ShootCommand(Conveyor conveyor, Shooter shooter, NoteSystemConfiguration conf, JoystickConfiguration j_conf, double rpm){
        ...
        Command conveyor_retract = new ConveyorSetPower(conveyor, conf.ShootConveyorReversePower, true).withTimeout(conf.ShootConveyorReverseTime);
        Command conveyor_push = new ConveyorSetPower(conveyor, conf.ShootConveyorPower, true).withTimeout(conf.ShootConveyorTime);
        Command shooter_stop = new ShooterSetRPM(shooter, 0, HoldMode.Indefinite);

        Command spin_up = shooter_rpm.alongWith(conveyor_retract);
        Command push_end = conveyor_push.andThen(shooter_rpm_hold).withTimeout(conf.PostShootHoldTime).andThen(shooter_stop).withTimeout(0.25);
